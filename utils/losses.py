...
def loss(args):
    hm_pred, r_pred, reg_pred, hm_true, r_true, reg_true, reg_mask, indices = args
    hm_loss = focal_loss(hm_pred, hm_true)
    r_loss = 0.05 * reg_l1_loss(r_pred, r_true, indices, reg_mask)
    reg_loss = reg_l1_loss(reg_pred, reg_true, indices, reg_mask)
    total_loss = hm_loss + r_loss + reg_loss
    return total_loss
